...
InvaderBody::InvaderBody() : cnv(GEO_INV_GRID_DIM * GEO_PIXEL_SIZE * 2, GEO_INV_GRID_DIM * GEO_PIXEL_SIZE * 2) {
  generatePattern();
  generateBitmap();
}
...
void InvaderBody::generatePattern() {
  ...
  for (uint8_t i = 0; i < GEO_INV_GRID_DIM; i++) {
    for (uint8_t j = 0; j < GEO_INV_GRID_DIM; j++) {
      invaderGrid[i][j] = false;
      invaderGrid[i][j] = softGrid[i][j] > threshold;
    }
  }
}
  ...
  for (int isMirrored = 0; isMirrored <= 1; isMirrored++) {
    for (int i = 0; i < GEO_INV_GRID_DIM; i++) {
      for (int j = 0; j < GEO_INV_GRID_DIM; j++) {
        if (invaderGrid[isMirrored ? GEO_INV_GRID_DIM - i - 1 : i][j]) {
